...
GCO_EXPORT(gco_getenergy)
{
	...
	mwSize outSize = 1;
	plhs[0] = mxCreateNumericArray(1, &outSize, cEnergyClassID, mxREAL);
	SiteID d_num = *(SiteID*)mxGetData(prhs[1])-1;
	LabelID g_num = *(LabelID*)mxGetData(prhs[2])-1;
	*(EnergyType*)mxGetData(plhs[0]) = gcinstance.gco->giveEnergy_esite(d_num,g_num);
}
