...
, m_giveSmoothEnergy_esite(0)
, m_giveDataEnergy_esite(0)
...
void GCoptimization::specializeDataCostFunctor(const UserFunctor f) {
	...
	m_giveDataEnergy_esite		= &GCoptimization::giveDataEnergy_esite<UserFunctor>;//YJ
}
...
void GCoptimization::specializeSmoothCostFunctor(const UserFunctor f) {
	...
	m_giveSmoothEnergy_esite	= &GCoptimization::giveSmoothEnergy_esite<UserFunctor>;//YJ
...
template <typename SmoothCostT>
GCoptimization::EnergyType GCoptimization::giveSmoothEnergy_esite(SiteID site, LabelID label)
{
	EnergyType eng = (EnergyType) 0;
	SiteID numN,*nPointer,nSite,n;
	EnergyTermType *weights;
	SmoothCostT* sc = (SmoothCostT*) m_smoothcostFn;

	giveNeighborInfo(site,&numN,&nPointer,&weights);
	for ( n = 0; n < numN; n++ )
	{
		nSite = nPointer[n];
		//if ( nSite < site ) 
		eng += weights[n]*(sc->compute(site,nSite,label,m_labeling[nSite]));
	}
...
GCoptimization::EnergyType GCoptimization::giveDataEnergy_esite(SiteID site, LabelID label)
{
	DataCostT* dc = (DataCostT*)m_datacostFn;
	EnergyType eng = (EnergyType) 0;
	eng=dc->compute(site, label);
	return eng;
}
//-------------------------------------------------------------------
...
void GCoptimization::setSmoothCostFunctor(SmoothCostFunctor* f) {
	...
	m_giveSmoothEnergy_esite	= &GCoptimization::giveSmoothEnergy_esite<SmoothCostFunctor>;//YJ
	//m_giveDataEnergy_esite		= &GCoptimization::giveDataEnergy_esite<SmoothCostFunctor>;//YJ	
...
GCoptimization::EnergyType GCoptimization::giveEnergy_esite(SiteID s, LabelID l)
{
	EnergyType energy = 0;
	energy = ((this->*m_giveSmoothEnergy_esite)(s, l))+((this->*m_giveDataEnergy_esite)(s, l));
	return energy;
}
...
void GCoptimizationGeneralGraph::setNeighbors(SiteID site1, SiteID site2, EnergyTermType weight)
{
	...
	int check1=0;
	int check2=0;
	m_neighbors[site1].setCursorFront();
	while(m_neighbors[site1].hasNext())
	{
		tmp = (Neighbor *) (m_neighbors[site1].next());
		if(tmp->to_node==temp1->to_node)check1=1;
	}
	m_neighbors[site2].setCursorFront();
	while(m_neighbors[site2].hasNext())
	{
		tmp = (Neighbor *) (m_neighbors[site2].next());
		if(tmp->to_node==temp2->to_node)check2=1;
	}
	if(check1==0)m_neighbors[site1].addFront(temp1);
	if(check2==0)m_neighbors[site2].addFront(temp2);
	*/
